...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('API Key length:', process.env.RIOT_API_KEY?.length || 0);
    ...
    if (!response.ok) {
      ...
      console.error('Riot API Error:', {
        ...
        url: `${regionUrl}${RIOT_API_CONFIG.TOURNAMENT_ENDPOINTS.REGISTER_PROVIDER}`,
        requestBody: providerData,
        errorData
      });
      return NextResponse.json(
        { 
          error: 'Failed to register provider with Riot API', 
          details: errorData,
          status: response.status,
          statusText: response.statusText,
          requestUrl: `${regionUrl}${RIOT_API_CONFIG.TOURNAMENT_ENDPOINTS.REGISTER_PROVIDER}`
        },
        { status: response.status }
      );
    }
